def setup_game(beallitasok):
    setup = True
    while setup is True:
        probalkozasok, feladvany_hossza, szinek_tobbszor, statisztika, szinek_utotag = int(), int(), bool(), bool(), str('\x1b[0m')
        ...
        if str(input()) == "i":
            ...
            pickle.dump(setup, config) #conf[7]
            config.close()
            setup = False
        else:
            continue