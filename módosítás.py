...
def check(bevitel:List,feladvany:List) -> List: 
    ...
    >>> ceck(bevitel = [1, 2, 3, 4], feladvany = [1, 2, 3, 4])
    [' OK ', ' OK ', ' OK ', ' OK ']

    >>> ceck(bevitel = [2, 4, 6, 7], feladvany = [4, 2, 7, 6])
    [' RH ', ' RH ', ' RH ', ' RH ']
...
def szinkod(bevitel:List) -> List:
    ...
    >>> szinkod(szinek = ["piros", "zold" , "sarga" , "kek" , "lila" , "cyan" , "feher"])
...
def tipp() -> List:
    ...
    >>> tipp([piros, feher, zold, kek])
    [1, 7, 2, 4]

    >>> tipp([rozsaszin, sarga, cyan, kek, zold])
    [ilyen szin nincs a jatekban, 3, 6, 4, 2]

    >>> tipp([cyan, feher, cyan, kek, lila])
    [6, 7, a cyan színt már tippelted, 4, 5]
